...
excerpt: "Control runs a vulnerable PHP web application that controls access to the admin page by checking the source IP address of the connection. By using the X-Forwarded-For HTTP header we can access the admin page and exploit an SQL injection to write a webshell and get a shell. After pivoting to another user with the credentials found in the MySQL database, we get SYSTEM access by modifying an existing service configuration from the registry."
date: 2020-04-25
classes: wide
header:
